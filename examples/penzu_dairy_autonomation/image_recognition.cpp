...
image_recognition::image_recognition(int x, int y) : mouse(x, y), keyboard() {
    // Additional constructor logic for image_recognition class
}
...
void image_recognition::temple_matching(string image_to_match, string screenshot_of_whole) {
    ...
    if (x_cord == 0 && y_cord == 0){
        cout << "template matching not successful" << endl;
    }
    else {
        cout << "template matching successful" << endl;
    }
}
